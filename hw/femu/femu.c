...
static int nvme_register_extensions(FemuCtrl *n)
{
    ...
    } else if (PINKSSD_EXP(n)) {
        nvme_register_pinkssd_exp(n);
    } else if (LKSV3SSD_EXP(n)) {
        nvme_register_lksv3ssd_exp(n);
    } else {
    }
...
static void femu_realize(PCIDevice *pci_dev, Error **errp)
{
    ...
    nvme_register_dram_backend(n);
    n->be_ops.init(&n->be, bs_size, n->femu_mode);
    n->be->femu_mode = n->femu_mode;
    ...
    n->throughput_cnt = 0;
    n->throughput_end = 0;
    n->throughput_interval_ns = 10000000000;
    n->latency_idx = 0;
    n->latency_sample_cnt = 0;
    n->latency_sample_interval_bound = 4000; // sample interval is up to 4K
    n->latency_sample_interval = n->latency_sample_interval_bound / 2;
...
static Property femu_props[] = {
    ...
    DEFINE_PROP_BOOL("kv_learned", FemuCtrl, kv_learned, false),
