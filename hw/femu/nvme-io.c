...
static void nvme_post_cqe(NvmeCQueue *cq, NvmeRequest *req)
{
	...
	// TODO: should we overwrite the nvme result somewhere else?
	cqe->n.result = cpu_to_le32(req->result);
...
static void nvme_process_cq_cpl(void *arg, int index_poller)
{
    ...
    if (log_started) {
        ...
        if (qatomic_read(&n->throughput_end) == 0) {
            qatomic_set(&n->throughput_end, now + n->throughput_interval_ns);
        }
        uint64_t end = qatomic_read(&n->throughput_end);
        if (end < now) {
            uint64_t old = qatomic_cmpxchg(&n->throughput_end, end, end + n->throughput_interval_ns);
            if (old == end) {
                uint64_t thru_cnt = qatomic_xchg(&n->throughput_cnt, 0);
                uint64_t write_cnt = qatomic_xchg(&n->write_cnt, 0);
                uint64_t comp_read_cnt = qatomic_xchg(&n->comp_read_cnt, 0);
                uint64_t comp_write_cnt = qatomic_xchg(&n->comp_write_cnt, 0);
                uint64_t host_read_cnt = qatomic_xchg(&n->host_read_cnt, 0);
                uint64_t host_write_cnt = qatomic_xchg(&n->host_write_cnt, 0);
                uint64_t gc_read_cnt = qatomic_xchg(&n->gc_read_cnt, 0);
                uint64_t gc_write_cnt = qatomic_xchg(&n->gc_write_cnt, 0);
                femu_debug("[Throughput] %lu\n", thru_cnt / (n->throughput_interval_ns / 1000000000));
                femu_debug("[Write_cnt] %lu\n", write_cnt);
                femu_debug("[Comp_read] %lu\n", comp_read_cnt);
                femu_debug("[Comp_write] %lu\n", comp_write_cnt);
                femu_debug("[GC_read] %lu\n", gc_read_cnt);
                femu_debug("[GC_write] %lu\n", gc_write_cnt);
                femu_debug("[Host_read] %lu\n", host_read_cnt);
                femu_debug("[Host_write] %lu\n", host_write_cnt);
                int i;
                for (i = 0; i < 128; i++) {
                    if (n->flash_access_counts[i] == 0) {
                        continue;
                    }
                    femu_debug("[Flash access count: %d] %lu\n", i, n->flash_access_counts[i]);
                }

                // femu_debug("[Hash_collision] %lu\n", qatomic_read(&n->hash_collision_cnt));
                // for (i = 0; i < 64; i++) {
                //    femu_debug("[Hash_collision][%.3d] %lu\n", i, qatomic_read(&n->hash_ssd_collision_cnt[i]));
                // }
            }
        }
    }

    if (BBSSD(n) || PINKSSD_EXP(n) || LKSV3SSD_EXP(n)) {
    ...
    while ((req = pqueue_peek(pq))) {
        ...
        if ((req->cmd_opcode == NVME_CMD_KV_RETRIEVE) && (qatomic_read(&n->latency_idx) < 10000)) {
            if (!log_started) {
                qatomic_set(&n->write_cnt, 0);
                qatomic_set(&n->comp_read_cnt, 0);
                qatomic_set(&n->comp_write_cnt, 0);
                qatomic_set(&n->gc_read_cnt, 0);
                qatomic_set(&n->gc_write_cnt, 0);
                qatomic_set(&n->host_read_cnt, 0);
                qatomic_set(&n->host_write_cnt, 0);
                qatomic_set(&n->hash_collision_cnt, 0);
                for (int i = 0; i < 256; i++) {
                    qatomic_set(&n->hash_ssd_collision_cnt[i], 0);
                }
                uint64_t new_interval = rand() % n->latency_sample_interval_bound;
                if (new_interval == 0)
                    new_interval = 1;
                qatomic_set(&n->latency_sample_interval, new_interval);
                log_started = true;
            }

            uint64_t current_cnt = qatomic_inc_fetch(&n->latency_sample_cnt);
            if (current_cnt == n->latency_sample_interval) {
                uint64_t idx = qatomic_inc_fetch(&n->latency_idx);
                femu_debug("[Latency %lu] %lu\n", idx, req->reqlat);

                qatomic_inc(&n->flash_access_counts[req->flash_access_count]);
                uint64_t new_interval = rand() % n->latency_sample_interval_bound;
                if (new_interval == 0)
                    new_interval = 1;
                qatomic_set(&n->latency_sample_interval, new_interval);
                qatomic_set(&n->latency_sample_cnt, 0);
            }
        }
        if (log_started) {
            if (req->cmd_opcode == NVME_CMD_KV_STORE || req->cmd_opcode == NVME_CMD_KV_RETRIEVE) {
                qatomic_inc(&n->throughput_cnt);
            }
        }
...
static uint16_t nvme_io_cmd(FemuCtrl *n, NvmeCmd *cmd, NvmeRequest *req)
{
	...
	// FIXME: KVSSD doesn't handling the namespace?
	if (nsid == 0) nsid = 1;
