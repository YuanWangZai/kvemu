...
void pink_lsm_create(void)
{
    ...
    kv_debug("SHOWINGSIZE(GB) :%lu HEADERSEG:%d DATASEG:%d\n", ((unsigned long) pink_ssd->sp.tt_pgs * 9 / 10 * PAGESIZE) / G, pink_ssd->sp.meta_lines, pink_ssd->sp.data_lines);
    kv_debug("LEVELN:%d\n", LSM_LEVELN); 
...
uint8_t lsm_find_run(kv_key key, pink_level_list_entry **entry, keyset **found, int *level, NvmeRequest *req) {
    ...
    for(int i = *level; i < LSM_LEVELN; i++){
        entries = find_run(pink_lsm->disk[i], key, req);
        if(!entries) {
            continue;
        }

        if (!entries->buffer && entries->ppa.ppa == UNMAPPED_PPA) {
            entries = find_run2(pink_lsm->c_level, key, req);
            if (level) {
