...
enum NvmeIoCommands {
	...
	NVME_CMD_KV_ITERATE_REQUEST = 0xB1,
	NVME_CMD_KV_ITERATE_READ    = 0xB2,
	NVME_CMD_KV_APPEND          = 0x83,
	NVME_CMD_KV_EXIST           = 0xB3,
	NVME_CMD_KV_DUMP            = 0xB4,
};
...
enum NvmeStatusCodes {
	...
	NVME_KV_ERR_ITERATE_READ_EOF= 0x0093,
...
typedef struct FemuCtrl {
	...
	uint64_t        iterate_idx;

    uint64_t        flash_access_counts[1024];

    uint64_t        throughput_cnt;
    uint64_t        write_cnt;
    uint64_t        comp_read_cnt;
    uint64_t        comp_write_cnt;
    uint64_t        gc_read_cnt;
    uint64_t        gc_write_cnt;
    uint64_t        host_read_cnt;
    ...
    uint64_t        latency_sample_interval;
    uint64_t        latency_sample_interval_bound;
} FemuCtrl;
...
enum {
    ...
    FEMU_PINKSSD_EXP_MODE = 7,
    FEMU_LKSV3SSD_EXP_MODE = 12,
};
...
static inline bool PINKSSD_EXP(FemuCtrl *n)
{
    return (n->femu_mode == FEMU_PINKSSD_EXP_MODE);
}

static inline bool LKSV3SSD_EXP(FemuCtrl *n)
{
    return (n->femu_mode == FEMU_LKSV3SSD_EXP_MODE);
}
